...
export async function fetchLLMSummary(letter: DoctorsLetter): Promise<string> {
  ...
  const fields = [
    ...
    `Empfohlenes Procedere: ${letter.recommendedProcedure || ""}`,
    `Anamnese: ${letter.anamnesis || ""}`,
    `Bisherige Medikation: ${letter.previousMedication || ""}`,
  ...
  const nonEmptyFields = fields.filter(field => field.split(":")[1].trim() !== "");

  const infoText = nonEmptyFields.join("\n---\n");
